...
def experiment(n_ins=[7],n_hiddens=[64],overwrite=False,
    ...
    def test_layernorm(eps_ortho,n_in,n_hidden,learning_rate,seed=1):
        # Build the model
        print('... building layer-normalized orthogonality-constrained RNN')
        x = T.tensor3('x')
        y = T.ivector('y') # labels are a 1D vector of integers
        rng = numpy.random.RandomState(seed)
        model = recurrent.rnn_ortho(x,n_in,n_hidden,10,rng,
    ...
    # hyperparameter search for orthogonality penalty w/ layer normalization
    if any('norm' in s for s in to_run):
        eps_orthos = [10**x for x in range(-5,0)]
        for idx,(eps_ortho,n_in,n_hidden) in \
                enumerate(itertools.product(eps_orthos,
                                            n_ins,
                                            n_hiddens)):
            fn = lambda seed: test_layernorm(eps_ortho,n_in,n_hidden,
                                             learning_rate,seed)
            trn,vld,tst,mntrs = repeat_test(fn,repeated_exp)
            log_results('norm.csv',idx,eps_ortho,n_in,n_hidden,
                        trn,vld,tst,mntrs,overwrite)
        graph.make_all('norm.csv')
